...
//Now we will call xor() with the ids for the variables a (=2) and b (=3), it will return the id of the newly created node
...
// We need to create a variable b_not, which will be the negated result of the node b. 
...
EXPECT_EQ(manager.coFactorFalse(newNodeID, 6));
...
EXPECT_EQ(manager.coFactorTrue(newNodeID, 6));
...
EXPECT_EQ(manager.coFactorFalse(newNodeID, 4));
